...
func (h *ServerHandler) Register(c *fiber.Ctx) error {
	...
	_ = services.LogActivity(h.DB, models.ActivityTypeServerRegistered,
...
func (h *ServerHandler) ListIdle(c *fiber.Ctx) error {
	var servers []models.Server
	if err := h.DB.Where("role = ?", models.ServerRoleNone).
		Where("status = ?", models.ServerStatusReady).
