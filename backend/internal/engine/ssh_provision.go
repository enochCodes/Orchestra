...
func (h *SSHProvisionHandler) HandlePreflightCheck(ctx context.Context, t *asynq.Task) error {
	...
	h.DB.Model(&server).Update("status", models.ServerStatusPreflight)
	...
	status := models.ServerStatusReady
...
func (h *SSHProvisionHandler) setServerError(server *models.Server, msg string) {
	h.DB.Model(server).Updates(map[string]interface{}{
		"status":        models.ServerStatusError,
		"error_message": msg,
	})
