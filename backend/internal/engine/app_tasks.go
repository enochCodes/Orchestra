...
func (h *AppTaskHandler) HandleDeployAppTask(ctx context.Context, t *asynq.Task) error {
	...
	var count int64
	h.DB.Model(&models.Deployment{}).Where("application_id = ?", app.ID).Count(&count)
	...
	case models.DeploymentSourceGit:
	...
	case models.DeploymentSourceDocker:
	...
	case models.DeploymentSourceManual:
	...
	if app.SourceType != models.DeploymentSourceDocker {
...
func (h *AppTaskHandler) deployK8s(client *sshpkg.Client, dep *models.Deployment, app *models.Application, image, name, envArgs string) error {
...
func (h *AppTaskHandler) deploySwarm(client *sshpkg.Client, dep *models.Deployment, app *models.Application, image, name, envArgs, portMapping string) error {
...
func (h *AppTaskHandler) deployDocker(client *sshpkg.Client, dep *models.Deployment, app *models.Application, image, name, envArgs, portMapping string) error {
...
func (h *AppTaskHandler) buildEnvArgs(app models.Application) string {
	var parts []string
	if app.EnvVars.Production != nil {
		for k, v := range app.EnvVars.Production {
...
func (h *AppTaskHandler) failDeployment(dep *models.Deployment, app *models.Application, msg string) {
	h.appendLog(dep, fmt.Sprintf("ERROR: %s", msg))
	h.DB.Model(dep).Update("status", models.DeploymentStatusFailed)
	h.DB.Model(app).Update("status", "failed")
}

func (h *AppTaskHandler) appendLog(dep *models.Deployment, line string) {
