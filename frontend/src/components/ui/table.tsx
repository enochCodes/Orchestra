...
>(({ className, ...props }, ref) => (
    <tfoot
        ref={ref}
        className={cn(
            "border-t bg-zinc-900/50 font-medium [&>tr]:last:border-b-0 text-zinc-300",
            className
        )}
        {...props}
    />
...
>(({ className, ...props }, ref) => (
    <th
        ref={ref}
        className={cn(
            "h-10 px-4 text-left align-middle font-medium text-zinc-400 [&:has([role=checkbox])]:pr-0 uppercase text-[11px] tracking-wider",
            className
        )}
        {...props}
    />
