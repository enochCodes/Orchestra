...
func (h *MonitoringHandler) GetOverview(c *fiber.Ctx) error {
	...
	h.DB.Model(&model.Application{}).Count(&appCount)
	h.DB.Model(&model.Application{}).Where("status = ?", "running").Count(&runningApps)
	h.DB.Model(&model.Deployment{}).Count(&deploymentCount)
...
func (h *MonitoringHandler) GetSystemStatus(c *fiber.Ctx) error {
	...
	dbOK := true
	var n int64
	if err := h.DB.Model(&model.Server{}).Count(&n).Error; err != nil {
		dbOK = false
	}
