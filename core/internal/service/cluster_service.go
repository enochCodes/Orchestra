...
func (s *ClusterService) DesignCluster(input DesignClusterInput) (*model.Cluster, error) {
	// Validate manager server
	var manager model.Server
	if err := s.DB.First(&manager, input.ManagerServerID).Error; err != nil {
		return nil, fmt.Errorf("manager server not found: %w", err)
	}
	if manager.Status != model.ServerStatusReady {
		return nil, fmt.Errorf("manager server is not in 'ready' state (current: %s)", manager.Status)
	}

	// Defaults
	clusterType := model.ClusterType(input.Type)
	if clusterType == "" {
		clusterType = model.ClusterTypeK8s
	}

	cni := input.CNIPlugin
	if cni == "" && clusterType == model.ClusterTypeK8s {
		cni = "flannel"
	}
